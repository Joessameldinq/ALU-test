...
def alu_operation_4bit(A_bin, B_bin, opcode_bin):
    """
    Simulates the 4-bit ALU operation based on the opcode and returns result with flags (Zero_flag, Overflow_flag, Carry).
    """
    ...
    if opcode_bin in ['0000', '0001', '0010', '0011']:
        signed_result = 0
        if opcode_bin in ['0000', '0001', '0010']:
            ...
            if opcode_bin == '0000': 
                ...
                signed_result = B_signed -  A_signed
                
            if unsigned_res > MAX_UNSIGNED or unsigned_res < 0:
                carry = '1'
            ...
            signed_result = A_signed * B_signed
            result_bin = to_twos_complement_bin(signed_result, BITS)
        
        if signed_result > MAX_SIGNED or signed_result < MIN_SIGNED:
            overfolw_flag = '1'
    ...
    # --- Logic and Bit-wise Operations (0100 - 1111) ---
        ...
        result_bin = to_twos_complement_bin(-A_signed)
    ...
    return result_bin, zero_flag, overfolw_flag, carry
...
def generate_complete_truth_table(filename="alu_complete_truth_table.txt"):
    ...
    with open(filename, 'w') as f:
        # Write header
        header = "C[4]\tA[4]\tB[4]\tOutput[4]\tZero_flag\tOverflow_flag\tCarry"
        f.write(header + "\n")
        ...
        for opcode_val in range(16):
            ...
            for A_val in range(16):
                ...
                for B_val in range(16):
                    ...
                    output_bin, zero_flag, overflow_flag, carry_flag = alu_operation_4bit(A_bin, B_bin, opcode_bin)
                    
                    # Format row with tabs
                    row = f"{opcode_bin}\t{A_bin}\t{B_bin}\t{output_bin}\t\t{zero_flag}\t\t{overflow_flag}\t\t{carry_flag}"
                    f.write(row + "\n")
                    total_rows += 1
